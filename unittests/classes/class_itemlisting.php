<?php
	...
	class ItemListing 
	{
		...
		public function item($itemNumber, $itemName, $itemPrice, $itemDescription) 
		{
			...
			$log = $db->query("SELECT * FROM product INNER JOIN user ON product.UserId = user.UserId WHERE ProductId = '$itemNumber' AND ProductName = '$itemName' AND Price = '$itemPrice' AND ProductDetail = '$itemDescription'");
